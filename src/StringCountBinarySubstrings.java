 ...
 * 这里需要用到三个变量，记录当前数字的数量cur,记录与当前数字不同的前面数字pre,这里要注意是与当前数字不同的数字，记录0和1数量相同情况下组成的群组的个数result。其中pre初始化为0，cur初始化为1，然后从第二个数字开始遍历，如果当前数字和前面的数字相同，则cur自增1，否则pre赋值为cur，cur重置1。然后判断如果pre大于等于cur，res自增1
 */
