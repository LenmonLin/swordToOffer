 ...
 * 如果两个字符串长度相等：
 *      遍历两个字符串，用一个hash记录两个不相同字符串字符的下标，如果这个hash长度
 *      等于2，同时对应下标的字符相等，就符合题意，输出true;
 * 考虑特殊情况：
 *      Input: A = "aa", B = "aa" 返回也应该是true;
 *      假如起始时A和B就完全相等，那么只有当A中有重复字符出现的时候，才能返回 true。
 *      快速检测重复字符的方法就是利用 HashSet 的自动去重复功能，将A中所有字符存入
 *      HashSet 中，若有重复字符，那么最终 HashSet的大小一定会小于原字符串A的长度。
 */
public class StringBuddyStrings859LeetCode {
    ...
    public static void main(String[] args) {
        String  A = "ab";
        String B = "ca";
        boolean result = new StringBuddyStrings859LeetCode().buddyStrings(A,B);
