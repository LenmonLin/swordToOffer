 ...
 *     括两个数)。没多久,他就得到另一组连续正数和为100的序列:18,19,20,21,22。
 *      现在把问题交给你,你能不能也很快的找出所有和为S的连续正数序列? Good Luck!
 *  输出描述:
 *       输出所有和为S的连续正数序列。序列内按照从小至大的顺序，序列间按照开始数字
 *       从小到大的顺序
 * 基本思路：
 * 1、用两个指针small，big分别记录头尾，然后如果两者之和小于sum,则后移尾指针，增加
 * 求和的数的数量，如果两者之和大于sum，则后移头指针，减少求和的数的数量。
 * 2、因为这个序列至少要有两个数字，则序列中至少有一个数是小于sum/2,否则都大于sum/2，
 * 则不可能由两个数组成，两个都大于sum/2的和一定大于sum,所以我们一直增加small
 * 到(1+sum)/2为止;
 * @author LemonLin
