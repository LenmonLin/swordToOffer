 ...
 *给定两个字符串 A 和 B, 寻找重复叠加字符串A的最小次数，使得字符串B成为叠加后的字符串A的子串，如果不存
 * 在则返回 -1。
 ...
 *      B要能成为A的字符串，那么A的长度肯定要大于等于B，所以当A的长度小于B的时候，我们可以先进行重复A，
 *      直到A的长度大于等于B，并且累计次数cnt。那么此时我们用indexOf来找，看B是否存在A中，如果存在直接
 *      返回cnt。如果不存在，我们再加上一个A，再来找，这样可以处理这种情况A="abc", B="cab"，如果此时
 *      还找不到，说明无法匹配，返回-1.
 *      当给定A长度大于B时，有两种情况需要判断，一种是B非子串，返回-1，一种是A要复制一次，返回2;
 *      比如"aaaaaaaaaaaaaaaaaaaaaab"    "ba"
 */
public class StringrepeatedStringMatch {
    ...
    public static void main(String[] args) {
        ...
        String test2 = "ba";
        int result = new StringrepeatedStringMatch().repeatedStringMatch(test1,test2);
