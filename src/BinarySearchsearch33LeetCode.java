 ...
 *      否则去左半部分找。
 *回应：本题其实都能想到用二分法，但是看上去又没那么舒服的使用二分法。因为数组不
 * 是完全递增的。所以本题的策略是，先找到完全递增的那一部分，然后在这完全递增的部分
 * 用二分法，当然可能要寻找得值不在完全递增的那一半部分里面怎么办，本题的思路就是
 * 想完全递增的那一部分就好，然后剩下的情况直接else处理，这样就能倒逼求得不完全的
 * 复杂部分的情况。
 */
