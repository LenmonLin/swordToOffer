 ...
 * Input: "Hello World"
 * Output: 5
 *翻译：给定一个仅包含大小写字母和空格 ' ' 的字符串，返回其最后一个单词的长度。
 * 如果不存在最后一个单词，请返回 0 。
 *  解题思路：
 *      从后往前遍历，一边遍历一边记录长度，当遇到空格时输出长度，当遍历结束没有遇到空格，根据题意输出0。
 *      要注意考虑Input: "Hello "的情况，要先用trim 把两边空格消掉。再计算
 */
public class StringLengthofLastWord {
    ...
    public static void main(String[] args) {
        String test = "a ";
        int result = new StringLengthofLastWord().lengthOfLastWord(test);
