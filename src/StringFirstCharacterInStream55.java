 ...
 *解题思路：
 *        利用hash表，先扫描一遍输入字符串，key值是出现的字符，value值记录出现的字符的次数，第二遍顺序扫描
 *直接读出value值是1的字符
 *         解题要点：
 *             1、ASCII 码转字符；http://blog.csdn.net/myweishanli/article/details/38924481
 *             字符转ascII 码；
 *             2、256的数组，初始化为-1，当第一次出现赋值为在字符流中的位置，第二次出现更新为-2；考虑用hash
 *             表的时候，字符不要惯性的认为只有26个英文字符，还有大写，-、+等符号字符，所以根据字符的位数，应
 *             该是256位字符。
