 ...
 * 那么我们可以连着旅行 7 天：第 2 天、第 3 天、第 4 天、第 5 天、第 6 天、第 7 天和第
 * 8 天。
 ...
 * 在第 3 天，你花了 costs[1] = $7 买了一张为期 7 天的通行证，它将在第 3, 4, ..., 9
 * 天生效。
...
public class DPmincostTickets983LeetCode {
    public int mincostTickets(int[] days, int[] costs) {
        ...
        //i设置是用来遍历日期的，当日期刚好等于days中的出行日期，才需要进行动态规划
        // 比较，否则不需要出行，dp[i]=dp[i-1];
        for (int i = minDay; i <=maxDay ;i++ ) {
            //i设置是用来遍历日期的，当日期刚好等于days中的出行日期，就进行出行成本比较
            if (days[day]==i){
                //设置这么多if判断是为了初始化以及防止数组越界。
                if (i-1<0){
                    cost1=costs[0];
                }else {
    ...
    public static void main(String[] args) {
        ...
        int [] costs={2,7,15};
        System.out.println(new DPmincostTickets983LeetCode().mincostTickets(days, costs));
    }
}
