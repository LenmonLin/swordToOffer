 ...
 * 利用一个新开辟的数组来存储每次读取的给定的数组值，比如给定数组为{4，2，1，5，6，
 * 0，1}那么读取到4，就在新数组的下标为4的地方存入4，读取2，存入2，以此类推，存入之
 * 前判断一下数组中是否已经存在这个数，如果存在就找到了对应的一个重复的数
 * 思路二：时间复杂度O(n),空间复杂度O(1)
 * 如果这个数组中没有重复的数字，那么排序完数字i将出现在下标为i的位置；
 * 从头到尾依次扫描这个数组中的每个数字。当扫描到下标为i的数字时，用一个m来暂存，对
 * 比是不是等于i.
 *       如果是，接着扫描下一个数字。
 *       如果不是，再拿它和第m个数字进行比较。
